...
rule TEcount:
    ...
    params:
        ...
        TE_gtf = lambda wildcards: config.get('genomes', {}).get('references', {}).get(wildcards.genome, {}).get('TE_gtf'),
        gtf = lambda wildcards: config.get('genomes', {}).get('references', {}).get(wildcards.genome, {}).get('gtf'),
        TEcount = config.get('Procedure',{}).get('TEcount') or 'TEcount'
    log:
        logdir + "/{genome}/{sample_id}/TEcount.log"
    conda:
        "../TEtranscripts.yaml"
    container:
...
rule combine_TEcount:
    ...
    log:
        logdir + "/{genome}/all/TEtranscripts/combine_TEcount.log"
    run:
        log_path = str(log)
        try:
            open(log_path, 'w').close()
...
rule combine_TElocal:
    ...
    log:
        logdir + "/{genome}/../group/TEtranscripts/combine_TElocal.log"
    run:
        log_path = str(log)
        try:
            open(log_path, 'w').close()
