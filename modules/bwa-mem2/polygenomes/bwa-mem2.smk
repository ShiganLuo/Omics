...
rule bwaMem2_index:
    ...
    output:
        index = expand(
            outdir + "/index/{genome}/{genome}.{ext}",
            ext = BWAMEM2_IDX_SUFFIX
        )
    log:
        logdir + "/index/{genome}/bwa_mem2-index.log"
    threads: 15
    conda:
        "../bwa-mem2.yaml"
    container:
...
def get_bwaMem2_index(wildcards):
    ...
    return [f"{outdir}/index/{wildcards.genome}/{wildcards.genome}.{ext}" for ext in BWAMEM2_IDX_SUFFIX]
...
rule bwaMem2_alignment:
    ...
    log:
        logdir + "/{sample_id}/{genome}/bwa_mem2-alignment.txt"
    threads: 15
    conda:
        "../bwa-mem2.yaml"
    container:
    ...
    run:
        ...
        try:
            ...
            with open(log_path, "a") as f:
                f.write(f"Error occurred during bwa-mem2 alignment for sample {wildcards.sample_id} on genome {wildcards.genome}: {e}\n")
            logger.error(f"Error occurred during bwa-mem2 alignment for sample {wildcards.sample_id} on genome {wildcards.genome}: {e}")
            raise e
