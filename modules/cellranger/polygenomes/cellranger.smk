"""Cell Ranger module for single-cell RNA-seq analysis (polygenomes version).

Provides three rules:
  - cellranger_ref:   Build reference genome from FASTA + GTF (per genome)
...
rule cellranger_ref:
    ...
    output:
        ref_dir = directory(outdir + "/cellranger_ref/{genome}")
    log:
        logdir_ref + "/cellranger_ref/{genome}/cellranger_ref.log"
    threads: 16
    conda:
        "../cellranger.yaml"
    container:
    ...
    params:
        ...
        genome_name = lambda wildcards: config.get("genome", {}).get(wildcards.genome, {}).get("genome_name", wildcards.genome),
    ...
    run:
        ...
        try:
            ...
            rule_logger.info(f"Start cellranger_ref for genome {wildcards.genome} at {current_time}")
            ...
            with open(command_script, "w") as f:
                ...
                f.write(f'echo "Cell Ranger reference for genome {wildcards.genome} built at {str(output.ref_dir)} successfully"\n')
            shell(f"bash {command_script} >> {log_path} 2>&1")
        except Exception as e:
            with open(log_path, "a") as f:
                f.write(f"cellranger_ref failed for genome {wildcards.genome}: {e}\n")
            raise RuntimeError(f"cellranger_ref failed for genome {wildcards.genome}: {e}\n")
...
def get_input_for_cellranger_count(wildcards):
    ...
    # Use per-genome cellranger_ref_dir from config or build it
    cellranger_ref_dir_provided = config.get("genome", {}).get(wildcards.genome, {}).get("cellranger_ref_dir")
    if cellranger_ref_dir_provided and os.path.exists(cellranger_ref_dir_provided):
        ...
        genome_fasta = config.get("genome", {}).get(wildcards.genome, {}).get("fasta", "")
        genome_gtf = config.get("genome", {}).get(wildcards.genome, {}).get("gtf", "")
        genome_name = config.get("genome", {}).get(wildcards.genome, {}).get("genome_name", wildcards.genome)
        if genome_fasta and genome_gtf and os.path.exists(genome_fasta) and os.path.exists(genome_gtf):
            in_dict["cellranger_ref_dir"] = outdir + f"/cellranger_ref/{wildcards.genome}/{genome_name}"
        else:
            raise ValueError(f"Neither transcriptome reference nor FASTA/GTF provided for genome {wildcards.genome} (sample {wildcards.sample_id})")
...
rule cellranger_count:
    ...
    output:
        ...
        raw_matrix = directory(outdir + "/{genome}/{sample_id}/raw_feature_bc_matrix"),
        filtered_matrix = directory(outdir + "/{genome}/{sample_id}/filtered_feature_bc_matrix"),
        h5 = outdir + "/{genome}/{sample_id}/filtered_feature_bc_matrix.h5"
    log:
        logdir + "/{sample_id}/{genome}/cellranger_count.log"
    threads: 8
    conda:
        "../cellranger.yaml"
    container:
    ...
    run:
        ...
        try:
            ...
            current_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
            rule_logger.info(f"Start cellranger_count for sample {wildcards.sample_id} on genome {wildcards.genome} at {current_time}")
            sample_id = wildcards.sample_id
            sample_outdir = os.path.join(outdir, wildcards.genome, sample_id)
            ...
            command_script = os.path.join(outdir, wildcards.genome, f"cellranger_count_{sample_id}_{current_time}.sh")
            cmd = [
            ...
            with open(command_script, "w") as f:
                f.write("#!/usr/bin/env bash\nset -euo pipefail\n")
                f.write(f"cd {outdir}/{wildcards.genome}\n")
                f.write(" ".join(cmd) + "\n")
                ...
                f.write(f'echo "Cell Ranger count completed for sample {sample_id} on genome {wildcards.genome}"\n')
            shell(f"bash {command_script} >> {log_path} 2>&1")
        except Exception as e:
            with open(log_path, "a") as f:
                f.write(f"cellranger_count failed for genome {wildcards.genome}: {e}\n")
            raise RuntimeError(f"cellranger_count failed for genome {wildcards.genome}: {e}\n")
...
rule cellranger_to_h5ad:
    ...
    input:
        ...
        filtered_matrix = directory(outdir + "/{genome}/{sample_id}/filtered_feature_bc_matrix"),
    output:
        h5ad = h5ad_outdir + "/{genome}/{sample_id}/{sample_id}_cellranger.h5ad"
    log:
        logdir + "/{sample_id}/{genome}/cellranger_to_h5ad.log"
    threads: 1
    conda:
        "../cellranger.yaml"
    container:
    ...
    run:
        ...
        try:
            ...
            with open(log_path, "a") as f:
                f.write(f"cellranger_to_h5ad failed for genome {wildcards.genome}: {e}\n")
            raise RuntimeError(f"cellranger_to_h5ad failed for genome {wildcards.genome}: {e}\n")
