...
rule deepvariant_run:
    ...
    output:
        vcf = outdir + "/{genome}/{sample_id}/{sample_id}.vcf.gz",
        csi = outdir + "/{genome}/{sample_id}/{sample_id}.vcf.gz.csi",
        gvcf = outdir + "/{genome}/{sample_id}/{sample_id}.g.vcf.gz"
    log:
        logdir + "/{genome}/{sample_id}/deepvariant.log"
    threads: 8
    container:
    ...
    params:
        ...
        outdir_sample = lambda wildcards: outdir + f"/{wildcards.genome}/{wildcards.sample_id}"
    conda:
        "../deepvariant.yaml"
    container:
    ...
    run:
        try:
            ...
            logger.info(f"Start DeepVariant for sample {wildcards.sample_id} genome {wildcards.genome} at {current_time}")
            script = os.path.join(outdir, f"{wildcards.genome}/{wildcards.sample_id}/deepvariant_{current_time}.sh")
            cmd1 = [
                params.deepvariant,
            ...
            raise f"Error occurred while running DeepVariant for sample {wildcards.sample_id} genome {wildcards.genome}: {e}, you can check the log file {log[0]} for more details."

rule deepvariant_result:
    input:
        vcf = outdir + "/{genome}/{sample_id}/{sample_id}.vcf.gz",
        tbi = outdir + "/{genome}/{sample_id}/{sample_id}.vcf.gz.csi"
