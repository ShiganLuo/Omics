...
def get_input_somaticMutect2(wildcards):
    ...
    """
    in_dict = {}
    known_sites = config.get("genome", {}).get(wildcards.genome, {}).get("known_sites")
    interval = config.get("genome", {}).get(wildcards.genome, {}).get("interval")
...
rule somaticMutect2:
    ...
    output:
        vcf = outdir + "/{genome}/mutect2-vcf/{normal_sample_id}_vs_{experimental_sample_id}/{normal_sample_id}_vs_{experimental_sample_id}.vcf.gz"
    log:
        logdir + "/{genome}/all/gatk/{normal_sample_id}_{experimental_sample_id}/mutect2.log"
    conda:
        "../../gatk.yaml"
    container:
        sif("../../gatk.yaml")
    ...
    run:
        try:
            ...
            logger.info(f"Start somaticMutect2 for normal sample {wildcards.normal_sample_id} and experimental sample {wildcards.experimental_sample_id} genome {wildcards.genome} at {current_time}")
            script = os.path.join(outdir, f"{wildcards.genome}/mutect2-vcf/{wildcards.normal_sample_id}_vs_{wildcards.experimental_sample_id}/mutect2_{current_time}.sh")
            cmd = [
                params.gatk, "Mutect2",
            ...
            raise f"Error occurred while running somaticMutect2 for normal sample {wildcards.normal_sample_id} and experimental sample {wildcards.experimental_sample_id} genome {wildcards.genome}: {e}, you can check the log file {log[0]} for more details."
