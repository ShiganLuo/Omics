		...
		for pos in data.keys():
			...
			if (sum(modTable[isodecoder][pos].values()) >= misinc_thresh and (any(cov >= min_cov) or 'mito' in isodecoder) and pos-1 not in knownTable[isodecoder]): # misinc above threshold, cov above threshold and not previously known
				...
				if (max(modTable[isodecoder][pos].values()) / sum(modTable[isodecoder][pos].values()) > 0.97):
					...
					if (tRNA_dict[isodecoder]['sequence'][pos-1] == 'A' and list(modTable[isodecoder][pos].keys())[list(modTable[isodecoder][pos].values()).index(max(modTable[isodecoder][pos].values()))] == 'G' and anticodon and pos-1 == min(anticodon)):
						new_inosines_cluster[cluster].append(pos-1)
						new_inosines_isodecoder[isodecoder].append(pos-1)
				elif (max(modTable[isodecoder][pos].values()) / sum(modTable[isodecoder][pos].values()) <= 0.97 and not (anticodon and pos-1 == min(anticodon))):
...
# modification counting and table generation, and CCA analysis
	...
	cov_table_melt.to_csv(os.path.dirname(inputs) + "/" + inputs.split("/")[-1] + "_coverage.txt", sep = "	", index = False)
	log.info('Wrote _coverage.txt for {} ({} rows) to {}'.format(inputs, len(cov_table_melt), os.path.dirname(inputs) + "/" + inputs.split("/")[-1] + "_coverage.txt"))
