	...
	with open(sampleData, "r") as sampleData:
		for line in sampleData:
			...
			if not line.startswith("#"):
				...
				sample_name = fq.split("/")[-1].split(".single.fq.gz")[0] if ".single.fq.gz" in fq else fq.split("/")[-1].split(".fastq.gz")[0] if ".fastq.gz" in fq else fq.split("/")[-1].split(".fastq")[0]
				sample_dir = os.path.join(out_dir, "samples", sample_name) + "/"
				os.makedirs(sample_dir, exist_ok=True)

				if map_round == 1:
					unique_bam, librarySize, alignstats = mapReads(fq, genome_index_path, genome_index_name, snp_index_path, snp_index_name, threads, out_dir, snp_tolerance, keep_temp, mismatches, remap, sample_dir=sample_dir)
				elif map_round == 2:
...
def mapReads(fq, genome_index_path, genome_index_name, snp_index_path, snp_index_name, threads, \
	out_dir, snp_tolerance, keep_temp, mismatches, remap, sample_dir=None):
# map with or without SNP index and report initial map statistics

	# sample_dir: per-sample output directory for BAM files
	# out_dir: shared directory for logs and stats
	...
	if snp_tolerance:
		map_cmd = ["gsnap", zipped, "-D", genome_index_path, "-d", genome_index_name, "-V", snp_index_path, "-v", \
			snp_index_name, "-t", str(threads), "--split-output", sample_dir + output_prefix, "--format", "sam", \
			"--genome-unk-mismatch", "0",   fq]
		...
		map_cmd = ["gsnap", zipped, "-D", genome_index_path, "-d", genome_index_name, "-t", str(threads), \
			"--split-output", sample_dir + output_prefix, "--format", "sam", "--genome-unk-mismatch", "0", \
			  fq]
		map_cmd = list(filter(None, map_cmd))
		map_cmd[-1:-1] = mismatch_list
	...
	transloc_file = sample_dir + output_prefix + ".unpaired_transloc"
	if os.path.exists(transloc_file):
		readcount = int(pysam.view("-@",str(threads),"-c", transloc_file).strip())
		if readcount == 0:
			os.remove(transloc_file)
	...
	with open(out_dir + "mapping_stats.txt","a") as stats_out:
		align_pathlist = Path(sample_dir).glob(output_prefix + "*")
		for file in align_pathlist:
			if re.search("mult",file.name) and not re.search("bam", file.name):
				multi_count = int(pysam.view("-@",str(threads),"-F", "0x904", "-c", sample_dir + file.name).strip())
				if keep_temp or remap:
					cmd = ["samtools", "view", "-@", str(threads), "-bh", "-o", sample_dir + file.name + ".bam", sample_dir + file.name]
					subprocess.check_call(cmd)
					os.remove(sample_dir + file.name)
				elif not keep_temp or not remap:
					os.remove(sample_dir + file.name)
			elif re.search("uniq",file.name) and not re.search("bam", file.name):
				unique_count = int(pysam.view("-@",str(threads),"-c", sample_dir + file.name).strip())
				unique_bam = sample_dir + file.name + ".bam"
				ps = subprocess.Popen(["samtools", "view", "-@", str(threads), "-bh", sample_dir + file.name], stdout = subprocess.PIPE)
				...
				unmapped_count = int(pysam.view("-@",str(threads),"-c", sample_dir + file.name).strip())
				if keep_temp or remap:
					cmd = ["samtools", "view", "-@", str(threads), "-bh", "-o", sample_dir + file.name + ".bam", sample_dir + file.name]
					subprocess.check_call(cmd)
					os.remove(sample_dir + file.name)
