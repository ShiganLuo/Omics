...
# Get decoy database
# decoy_fasta resolved per-genome in rules

def get_input_for_search_engine(wildcards):
    in_dict = {}
    in_dict["mzml"] = mzML_dir + f"/{wildcards.sample_id}/{wildcards.sample_id}.mzML"
    if config.get("genome", {}).get(wildcards.genome, {}).get("decoy_fasta") and os.path.exists(config.get("genome", {}).get(wildcards.genome, {}).get("decoy_fasta")):
        in_dict["fasta"] = config.get("genome", {}).get(wildcards.genome, {}).get("decoy_fasta")
    else:
...
rule search_engine_comet:
    ...
    output:
        idxml = outdir + "/{genome}/{sample_id}/{sample_id}_comet.idXML"
    log:
        logdir + "/{genome}/{sample_id}/search_engine_comet.log"
    threads: 4
    conda:
        "../../openms.yaml"
    container:
...
rule search_engine_msgf:
    input:
        unpack(get_input_for_search_engine)
    output:
        idxml = outdir + "/{genome}/{sample_id}/{sample_id}_msgf.idXML"
    log:
        logdir + "/{genome}/{sample_id}/search_engine_msgf.log"
    threads: 4
    conda:
        "../../openms.yaml"
    container:
...
rule search_engine_sage:
    input:
        unpack(get_input_for_search_engine)
    output:
        idxml = outdir + "/{genome}/{sample_id}/{sample_id}_sage.idXML"
    log:
        logdir + "/{genome}/{sample_id}/search_engine_sage.log"
    threads: 4
    conda:
        "../../openms.yaml"
    container:
