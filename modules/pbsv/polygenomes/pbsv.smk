...
rule pbsv_discover:
    ...
    output:
        svsig = outdir + "/{genome}/{sample_id}/{sample_id}.svsig.gz"
    log:
        logdir + "/{genome}/{sample_id}/pbsv_discover.log"
    threads: 4
    params:
    ...
    run:
        ...
        logger.info(f"Start pbsv discover for sample {wildcards.sample_id} genome {wildcards.genome} at {current_time}")
        script = os.path.join(outdir, f"{wildcards.genome}/{wildcards.sample_id}/pbsv_discover_{current_time}.sh")
        cmd = [
...
rule pbsv_call:
    ...
    output:
        vcf_gz = outdir + "/{genome}/{sample_id}/{sample_id}.sv.vcf.gz",
        tbi = outdir + "/{genome}/{sample_id}/{sample_id}.sv.vcf.gz.csi"
    log:
        logdir + "/{genome}/{sample_id}/pbsv_call.log"
    threads: 8
    params:
        pbsv = config.get("Procedure", {}).get("pbsv") or "pbsv",
        bgzip = config.get("Procedure", {}).get("bgzip") or "bgzip",
        bcftools = config.get("Procedure", {}).get("bcftools") or "bcftools",
        vcf = lambda wildcards: outdir + f"/{wildcards.genome}/{wildcards.sample_id}/{wildcards.sample_id}.sv.vcf"
    conda:
        "../pbsv.yaml"
    container:
        sif("../pbsv.yaml")
    run:
        current_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
        logger.info(f"Start pbsv call for sample {wildcards.sample_id} genome {wildcards.genome} at {current_time}")
        script = os.path.join(outdir, f"{wildcards.genome}/{wildcards.sample_id}/pbsv_call_{current_time}.sh")
        cmd1 = [
            params.pbsv, "call",
            "--num-threads", str(threads),
            input.fasta,
...
rule pbsv_result:
    input:
        vcf = outdir + "/{genome}/{sample_id}/{sample_id}.sv.vcf.gz",
        tbi = outdir + "/{genome}/{sample_id}/{sample_id}.sv.vcf.gz.csi"
