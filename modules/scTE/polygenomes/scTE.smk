...
rule scTE_build_index:
    ...
    params:
        ...
        genome = _scTE_genome,
        mode = _scTE_mode,
        gene_gtf = config.get("genome", {}).get(wildcards.genome, {}).get("gtf"),
        te_bed = config.get("genome", {}).get(wildcards.genome, {}).get("te_bed"),
...
def get_input_for_scTE_quantify(wildcards):
    ...
    scTE_index = config.get("genome", {}).get(wildcards.genome, {}).get("scTE_index")
...
rule scTE_quantify:
    ...
    output:
        h5ad = outdir + "/{genome}/{sample_id}/{sample_id}_scTE.h5ad"
    log:
        logdir + "/{genome}/{sample_id}/scTE_quantify.log"
    threads: 8
    conda:
        "../scTE.yaml"
    container:
