...
rule somatic_spectrum:
    ...
    output:
        spectrum_png = outdir + "/{genome}/somatic_spectrum_stacked_bar.png"
    log:
        logdir + "/{genome}/all/spectrum/somatic_spectrum.log"
    params:
        spectrum_script = os.path.join(ROOT_DIR, "modules/spectrum/bin/spectrum.py"),
        outprefix = lambda wildcards: outdir + f"/{wildcards.genome}/somatic_spectrum"
    conda:
        "../spectrum.yaml"
    container:
        sif("../spectrum.yaml")
    run:
        try:
            ...
            script = f"{outdir}/{wildcards.genome}/somatic_spectrum.{current_time}.sh"
            experiment_vcf_map_file = os.path.join(outdir, wildcards.genome, "experiment_vcf_map.tsv")
            os.makedirs(os.path.dirname(experiment_vcf_map_file), exist_ok=True)
            ...
            experiment_group_map_file = os.path.join(outdir, wildcards.genome, "experiment_group_map.tsv")
            ...
            with open(log) as f:
                f.write(f"Error in somatic_spectrum rule for genome {wildcards.genome}: {str(e)}\n")
            raise e
