"""Canonical three-pass STAR alignment for small non-coding RNA reads (polygenomes)."""
...
rule star_3p_align:
    input:
        ...
        smallrna_index=lambda wildcards: config["genome"][wildcards.genome]["smallrna_index"],
        smallrna_bed=lambda wildcards: config["genome"][wildcards.genome]["smallrna_bed"],
    output:
        pass1_bam=outdir + "/{genome}/{sample_id}/{sample_id}_pass1.bam",
        pass1_bai=outdir + "/{genome}/{sample_id}/{sample_id}_pass1.bam.bai",
        pass1_fq1=outdir + "/{genome}/{sample_id}/{sample_id}_pass1_extract_1.fq.gz",
        pass1_fq2=outdir + "/{genome}/{sample_id}/{sample_id}_pass1_extract_2.fq.gz",
        pass2_bam=outdir + "/{genome}/{sample_id}/{sample_id}_pass2.bam",
        pass2_bai=outdir + "/{genome}/{sample_id}/{sample_id}_pass2.bam.bai",
        pass2_unmapped1=outdir + "/{genome}/{sample_id}/{sample_id}_pass2_unmapped_fq_1.fq.gz",
        pass2_unmapped2=outdir + "/{genome}/{sample_id}/{sample_id}_pass2_unmapped_fq_2.fq.gz",
        pass3a_bam=outdir + "/{genome}/{sample_id}/{sample_id}_pass3a.bam",
        pass3a_bai=outdir + "/{genome}/{sample_id}/{sample_id}_pass3a.bam.bai",
        pass3b_bam=outdir + "/{genome}/{sample_id}/{sample_id}_pass3b.bam",
        pass3b_bai=outdir + "/{genome}/{sample_id}/{sample_id}_pass3b.bam.bai",
        bam=outdir + "/{genome}/{sample_id}/{sample_id}.bam",
        bai=outdir + "/{genome}/{sample_id}/{sample_id}.bam.bai",
    log:
        logdir + "/{sample_id}/{genome}/star3p.log"
    threads: 12
    conda:
        "../../star.yaml"
    container:
    ...
    run:
        ...
        try:
            ...
            rule_logger.info(f"Start star_3p_align for sample {wildcards.sample_id} genome {wildcards.genome} at {current_time}")
            ...
            rule_logger.info(f"star_3p_align for sample {wildcards.sample_id} genome {wildcards.genome} completed")
        except Exception as e:
            with open(log_path, "a") as f:
                f.write(f"star_3p_align failed for sample {wildcards.sample_id} genome {wildcards.genome}: {e}\n")
            raise e
